...
export type DashboardMode = "preview" | "simulator"

type EmptyStateDashboardProps = {
  mode: DashboardMode
      ...
      <div className="space-y-3 rounded-xl border p-6">
        ...
        <div className="inline-flex items-center gap-2 rounded-full border p-1">
          ...
          <button
            type="button"
            className={`rounded-full px-3 py-1 text-sm ${
              mode === "simulator" ? "bg-foreground text-background" : ""
            }`}
        ...
        >
          ...
          {calculatorFireNumber !== null && yearsToTarget !== null ? (
            <p className="text-muted-foreground text-sm">
              {mode === "simulator"
                ? `Simulator preview: this scenario targets financial independence in about ${yearsToTarget} years based on current assumptions.`
                : `Preview mode: this plan targets financial independence in about ${yearsToTarget} years based on the current calculator inputs.`}
            </p>
          ) : null}
        </div>
